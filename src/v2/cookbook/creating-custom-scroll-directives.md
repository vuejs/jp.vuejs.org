...
要素に対してスクロールイベントの着脱は本当に良いユースケースです。なぜなら他のディレクティブと同様に、要素に結びつけられる必要があり、DOM を参照しなければならないためです。このパターンはトラバーサルを回避し、参照先のノードとイベントロジックのペアを保つことができます。

## 実例: カスケードアニメーションでのカスタムスクロールディレクティブの使用

一貫したサイトを作成する過程で、様々な箇所で同じアニメーションロジックを再利用していることに気づくかもしれません。シンプルに思えますが、その時とても限定的なカスタムディレクティブを作成していましたよね？通常通りにそれを再利用するならば、使用の度に「ほんの」些細な変更を必要とすることでしょう。
...
上のデモで、それぞれのセクションはスクロールによって引き起こされる、2つの異なるアニメーションがあります。変形アニメーションと、SVG内の個々のパスに沿って動く描画アニメーションです。これら2つのアニメーションを再利用するため、それぞれのカスタムディレクティブを作成します。カスタムディレクティブに渡すであろう引数は、シンプルかつ再利用可能なコードの維持に役立ちます。
...
それではテンプレートでこのアニメーションを使用しましょう。この場合、`clipPath`要素にディレクティブを加え、全ての引数をオブジェクトとしてディレクティブに渡します。
...
カスタムディレクティブは非常に役に立ちますが、スクラッチでは作りたくない、スクロールライブラリにすでに存在する特殊な機能を使いたい場合もあるかもしれません。
