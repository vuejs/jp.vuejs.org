...
- **使用方法:**
  ...
  > 2.1.0 から新規: コールバックが提供されず、実行環境で Promise がサポートされている場合は Promise を返します。
...
- **詳細:**
  ...
  プロパティとして認識(および抽出)されない親スコープの属性バインディング(except for `class` and `style`)が含まれています。コンポーネントに宣言されたプロパティがない場合、(`class` と `style` 以外の)全ての親スコープのバインディングが基本的に含まれ、`v-bind="$attrs"` 経由で内部コンポーネントに渡すことができます。高次 (higher-order) コンポーネントを作成するときに便利です。
