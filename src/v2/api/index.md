...
- **型:** `{ [key: string]: Function | { get: Function, set: Function, cache: Boolean } }`
...
- **詳細:**
  ...
  算出プロパティはキャッシュされ、そしてリアクティブ依存が変更されたときにだけ再算出します。あるの依存関係がインスタンスのスコープ外の(つまりリアクティブではない)場合、算出プロパティは更新されないことに注意してください。このような状況では、`cache: false` を設定することでキャッシングをオフにすることができますが、依存関係は依然としてリアクティブではないため、それを変更しても DOM の更新はトリガーされません。

  ほとんどの場合、`cache: false` は理想的な解決策ではありません。可能な限り、外部データをリアクティブシステムに取り込む方がはるかに優れています。例えば、算出プロパティがウィンドウのサイズに依存する場合、この情報を `data` に格納し、` resize` イベントを使用して値を最新の状態に保つことができます。まさしくリアクティブです！
...
- **詳細:**
  ...
  現在のコンポーネントツリーのルート Vue インスタンスです。現在のインスタンスが親ではない場合、この値はそれ自身でしょう。
