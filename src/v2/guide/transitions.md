...
6. `v-leave-to`: **バージョン 2.1.8 以降でのみ利用可能です。** leave の終了状態。leaving トランジションがトリガされた 1 フレーム後に追加され (同時に `v-leave` が削除されます)、トランジション/アニメーションが終了すると削除されます。
...
- CSS トランジションクラスは、グループ/コンテナ要素自身ではなく、中の要素に適用されます。
