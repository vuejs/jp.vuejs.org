...
Vue はトランジションが終了したことを把握するためのイベントリスナーのアタッチを必要とします。イベントは、適用される CSS ルールに応じて `transitionend` か `animationend` のいずれかのタイプになります。あなたがトランジションとアニメーション、どちらか一方だけ使用する場合は、Vue は自動的に正しいタイプを判断することができます。
...
あとで [コンポーネント間のトランジション](#コンポーネント間のトランジション) について説明しますが、`v-if`/`v-else` を使った通常の要素同士でもトランジションできます。最も共通の2つの要素のトランジションの例として、リストコンテナとリストが空と説明するメッセージの間でおこなうものがあります:
...
それは、"on" ボタンと "off" ボタン間でトランジションをおこなうとき、片方のボタンがトランジションアウトして、別の片方がトランジションインするとき、両方のボタンが描画されてしまうことです。これは、`<transition>` のデフォルトの振る舞いです - entering と leaving は同時に起きます。
...
- `<transition>` と異なり、実際に要素を描画します: `<span>` がデフォルトです。この要素は、`tag` 属性で変えることができます。
