...
- 独立したノード
- 複数ノードのうち一度に1つのみをレンダリング

それでは、例えば、`v-for` のように同時にレンダリングしたいアイテム・リストがある場合はどうすればよいでしょう？ この場合は、`<transition-group>` コンポーネントを使うことができます。例を詳しく見ていく前に、このコンポーネントについて知っておくべき幾つかの重要なことをあげておきましょう。
