...
いくつかの HTML 要素は、例えば `<table>` は、要素がその要素内部に表示可能かの制限があります。ホワイトリストに含まれていないカスタム要素は表示出来ない要素としてコンパイル処理で一致するため、したがって適切にレンダリングしません。そのようなケースでは、`is` という特別な属性でカスタム要素に示す必要があります:
