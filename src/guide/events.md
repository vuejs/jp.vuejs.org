...
`v-on` ディレクティブを使うことで、DOMイベントの購読、イベントが発火時の JavaScript を実行ができます。
...
多くのイベントハンドラのロジックはよく複雑になっていくので、`v-on` 属性の値に JavaScript 式を記述し続けるのは現実的ではありません。そのため、`v-on` は他にも呼び出したいメソッドの名前を受け付けます。
...
時には、インラインステートメントハンドラでオリジナルの DOM イベントを参照する必要があります。特別な `$event` 変数を使うことでメソッドに渡すことができます:
...
必要であれば、カスタムキー修飾子の別名を定義することもできます:
...
これまで説明してきたようなイベント監視のアプローチは、"関心の分離"という古き良きルールを破っているのではないか、と心配されるかもしれません。安心してください。すべての Vue ハンドラ関数と式は、現在の View を扱う ViewModel に厳しく制限されています。それによってメンテナンスが難しくなることはありません。実際、`v-on` を利用することでいくつかの利点があります。
