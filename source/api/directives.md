...
0.12.8 以降では、input 要素で `value` 属性プロパティが使用された場合、`v-attr` はその要素に対応する `value` プロパティを設定します。例えば、`<input value="{% raw %}{{val}}{% endraw %}">` は属性を更新するだけではなく、内在する JavaScript のプロパティも設定します。
