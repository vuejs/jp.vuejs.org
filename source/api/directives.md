...
0.12.8 以降では、プロパティが存在する場合、`v-attr` は要素に対応するプロパティを設定します。例えば、`<input value="{% raw %}{{val}}{% endraw %}">` は属性を更新するだけではなく、value プロパティも設定します。もし、要素がバインドされた属性に対応するプロパティを持っていない場合は、設定しません。
