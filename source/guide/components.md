...
デフォルトで、全ての props は子プロパティと親プロパティとの間で双方向バインディングです。親プロパティが更新したとき、子までに同期され、そして逆もしかりです。しかしながら、mustache の先頭に`*` を追加することによって、親から子へ同期だけで一方向バインディングを強制することも可能です:
...
加えて、もし親 prop の expression が "設定可能" ではない場合、バインディングは自動的に一方向になります。例:
