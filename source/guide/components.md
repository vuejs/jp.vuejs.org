...
コンポーネントは受け取る props の検証することができます。これは他の人に使用されるために目的とされたコンポーネントを編集するときに便利で、これらの prop 検証要件は本質的にはコンポーネントの API を構成するものとして、ユーザーがコンポーネントを正しく使用しているということを保証します。文字列として定義している props の代わりに、検証要件を含んだオブジェクトを使用できます:
...
もし props が任意の検証を必要としない場合は、今までどおり文字列を使用でき、そして配列に文字列とオブジェクト props  を混在できます。
