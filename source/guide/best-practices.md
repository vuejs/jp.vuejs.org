...
もし、`created` フックでコンポーネントの props にアクセスしようとうした場合、`undefined` としてそれらを見つけるでしょう。これはインスタンスに対して全ての DOM のコンパイルが発生する前に `created` フックが呼ばれてしまうためで、従ってまだ Props が処理されていないからです。Props テンプレートのコンパイル**後**に親の値で初期化されます。同様に two-way バインドされた props はコンパイル後、親の変更だけトリガできます。
